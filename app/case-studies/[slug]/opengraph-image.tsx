...
const visualSpecs: Record<string, VisualSpec> = {
  ...
  "workshop-management-system": {
    ...
    surface: "HR portal + waitlist + AI drafts",
    primary: "Enterprise workshop ops with approved AI notes",
    secondary: "Capacity, waitlists, Google Workspace reminders, and insight drafts behind approval.",
    modules: ["Publish", "Register", "Waitlist", "Insights"],
    flow: ["HR publish", "Employee register", "Remind", "Summarize"],
    metrics: ["Hard capacity", "FIFO waitlist", "HITL AI"],
    evidence: ["Postgres schema", "Workspace adapters", "Approval gate"],
  },
  "mike-legal-ai": {
    layout: "agenticAi",
    product: "Mike Legal AI",
    surface: "Ingest + convert + query",
    primary: "Authenticated legal document workspace",
    secondary: "Office files convert to PDF, then teams review and query the corpus in one product.",
    modules: ["Auth", "Storage", "Conversion", "Query"],
    flow: ["Upload", "Convert", "Index", "Ask"],
    metrics: ["Signed-in", "Office files", "Model-backed"],
    evidence: ["Supabase auth", "Object storage", "LibreOffice path"],
  },
  "magic-creation-studio-wedding-photography-erp": {
    layout: "serviceMarketplace",
    product: "Wedding photography ERP",
    surface: "Bookings + shoots + albums",
    primary: "Studio OS for photography operations",
    secondary: "A signed-in product for bookings, shoot days, album delivery, and studio admin.",
    modules: ["Clients", "Shoots", "Albums", "Admin"],
    flow: ["Book", "Shoot", "Edit", "Deliver"],
    metrics: ["Signed-in", "Operational", "Studio-ready"],
    evidence: ["Supabase app", "Role access", "Delivery status"],
  },
  "hospital-voice-agent": {
    layout: "voiceAgent",
    product: "Hospital voice agent",
    surface: "Call + language + calendar",
    primary: "Trilingual inbound receptionist for hospitals",
    secondary: "Hindi, Odia, and English calls that book a Google Calendar slot.",
    modules: ["STT", "Agent", "TTS", "Calendar"],
    flow: ["Call", "Understand", "Slot", "Confirm"],
    metrics: ["3 languages", "Inbound", "Calendar write"],
    evidence: ["Sarvam voice", "Groq tools", "Calendar API"],
  },
  "hiredesk-recruiting-match-system": {
    layout: "jobsBoard",
    product: "Hiredesk match system",
    surface: "Resume + job ingest + rank",
    primary: "Recruiting backend that scores fit",
    secondary: "Parse resumes, ingest jobs, embed, then rerank matches with an LLM.",
    modules: ["Parse", "Ingest", "Embed", "Rerank"],
    flow: ["Resume", "Profile", "Jobs", "Match"],
    metrics: ["Workers", "Embeddings", "Rerank"],
    evidence: ["Celery ingest", "Postgres matches", "LLM rerank"],
  },
  "ad-sakhi-ad-ops-studio": {
    layout: "leadAutomation",
    product: "ad sakhi",
    surface: "URL + 9:16 creative + spend",
    primary: "Tiny ad-ops studio on one desk",
    secondary: "Paste a product URL, make a vertical ad, then watch spend from the same login.",
    modules: ["Make", "Spend", "Lines", "Auth"],
    flow: ["URL", "Creative", "Publish", "Spend"],
    metrics: ["9:16", "Signed-in", "Demo mode"],
    evidence: ["Creative factory", "Clerk desk", "Spend view"],
  },
  "bidsure-ai-tender-eligibility-platform": {
    layout: "agenticAi",
    product: "BidSure AI",
    surface: "Tender + eligibility + bid",
    primary: "Contractor desk for tenders they can actually bid",
    secondary: "Eligibility checks run before bid assembly, on structured requirements.",
    modules: ["Intake", "Eligibility", "Requirements", "Bid"],
    flow: ["Tender", "Check", "Qualify", "Assemble"],
    metrics: ["Eligibility first", "Structured", "Bid workspace"],
    evidence: ["Requirement model", "Eligibility rules", "Bid record"],
  },
  "brand-sahayak-social-content-platform": {
    layout: "whatsappAutomation",
    product: "Brand Sahayak",
    surface: "Generate + translate + schedule",
    primary: "Social content ops with a brand glossary",
    secondary: "Captions, translation, scheduling, and workspace RLS in one desk.",
    modules: ["Generate", "Glossary", "Schedule", "Analytics"],
    flow: ["Brief", "Draft", "Translate", "Schedule"],
    metrics: ["AI copy", "Glossary", "Workspaces"],
    evidence: ["OpenAI drafts", "Supabase RLS", "Calendar"],
  },
  "siddhi-vinayak-tiles-3d-showroom-website": {
    layout: "luxurySite",
    product: "Siddhi Vinayak Tiles",
    surface: "3D wall + room visualizer",
    primary: "Interactive stone and tile showroom",
    secondary: "Live floor retexturing, gallery, and WhatsApp contact for a Nuapada showroom.",
    modules: ["3D wall", "Visualizer", "Gallery", "WhatsApp"],
    flow: ["Enter", "Pick material", "See room", "Call"],
    metrics: ["WebGL", "Fallback", "Config-driven"],
    evidence: ["Three.js room", "Swatch set", "Static fallback"],
  },
  "relo-packers-and-movers-service-website": {
    layout: "serviceSite",
    product: "Relo Packers and Movers",
    surface: "Services + inquiry",
    primary: "Local moving site with a direct quote path",
    secondary: "A fast Vite front for packing, moving, and inquiry instead of a heavy CMS.",
    modules: ["Hero", "Services", "Coverage", "Inquiry"],
    flow: ["Visit", "Service", "Quote", "Call"],
    metrics: ["Fast front", "Mobile", "Inquiry path"],
    evidence: ["Service pages", "Contact path", "Vite build"],
  },
};
